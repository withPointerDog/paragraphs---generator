...
form.addEventListener("submit", (event) => {
  ...
  if (isNaN(inputNum) || inputNum > 9 || inputNum < 0) {
    paragraph.innerHTML = `<p class="result">${texts[random]}</p>`;
  } else {
    let resultTexts = texts.slice(0, inputNum);
    resultTexts = resultTexts
      .map(function (item) {
        return `<p class="result">${item}</p>`;
      })
      .join("");
    resultTexts.innerHTML = resultTexts;
  }
});
